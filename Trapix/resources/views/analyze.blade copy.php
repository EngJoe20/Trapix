                ...
                <div class="lg:col-span-5 flex flex-col gap-6 fade-in-up stagger-delay-1">
                    ...
                    <div class="glass-panel p-8 relative overflow-hidden group" id="dropZone"
                        ...
                        <div class="glass-panel p-8 relative overflow-hidden group" id="dropZone"
                            ...
                            <!-- Input for Files & Compressed Folders (مدخل الملفات والمجلدات المضغوطة) -->
                            <input type="file" id="fileInput" class="hidden" multiple @change="handleFileSelect($event)">

                            <!-- Input for Uncompressed Folders (مدخل المجلدات العادية) -->
                            <input type="file" id="folderInput" class="hidden" webkitdirectory directory
                                @change="handleFileSelect($event)">

                            <!-- Dropzone Default -->
                            <div class="text-center" x-show="!selectedFile && !isUploading && !isCompleted">
                                <div class="w-16 h-16 mx-auto mb-4 rounded-full border-2 border-primary/30 flex items-center justify-center group-hover:border-primary transition-colors cursor-pointer"
                                ...
                                <p class="text-body text-sm mb-3">Drag & drop anywhere inside the box, or:</p>

                                <!-- Dual Browse Buttons (أزرار التصفح المزدوجة) -->
                                <div class="flex justify-center items-center gap-3">
                                    <button type="button" @click.stop="document.getElementById('fileInput').click()"
                                        class="text-sm font-semibold text-green-500 hover:text-green-400 underline decoration-2 underline-offset-4 transition-colors">
                                        Browse Files / ZIP
                                    </button>
                                    <span class="text-body text-xs opacity-50">|</span>
                                    <button type="button" @click.stop="document.getElementById('folderInput').click()"
                                        class="text-sm font-semibold text-green-500 hover:text-green-400 underline decoration-2 underline-offset-4 transition-colors">
                                        Browse Folders
                                    </button>
                                </div>
                                <p class="text-body text-xs mt-4 opacity-70">(Max 100MB)</p>
                            </div>

                            <!-- File Selected UI (واجهة الملف المختار) -->
                            <div x-show="selectedFile && !isUploading && !isCompleted" style="display: none;"
                                ...
                                <div
                                    ...
                                    <svg class="w-8 h-8 text-green-400" fill="none" viewBox="0 0 24 24"
                                        stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
