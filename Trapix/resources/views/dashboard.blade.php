        ...
        <div class="max-w-7xl mx-auto relative z-10" x-data="dashboardApp()">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 fade-in-up">
                <div>
                    ...
                    <p class="text-body text-sm">Welcome back, {{ $user->name }} • Plan: <span
                            class="text-green-400 font-semibold uppercase">{{ $plan?->name ?? 'Free' }}</span></p>
                </div>
                ...
                <div class="glass-card p-6">
                    ...
                    @else
                        @php
                            ...
                            $color = $percent > 90 ? 'bg-red-500' : ($percent > 70 ? 'bg-amber-500' : 'bg-green-500');
                        @endphp
                        ...
                        <p class="text-xs text-body">Resets on
                            {{ $user->quota_reset_date ? $user->quota_reset_date->format('M j, Y') : '1st of month' }}
                        </p>
                    @endif
                </div>
                    ...
                    <ul class="space-y-2 text-sm text-body-contrast">
                        ...
                        <li class="flex items-center gap-2 mt-3">
                            <a href="{{ route('pricing') }}"
                                class="text-green-400 hover:text-green-300 underline text-xs">Upgrade Plan</a>
                        </li>
                    </ul>
                </div>
            ...
            <!-- History Table -->
            <div id="history" class="glass-panel p-6 fade-in-up stagger-delay-2">

                <div
                    class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 border-b border-box-border pb-4">
                    <h3 class="text-xl font-bold text-heading-1">Analysis History</h3>

                    <div class="flex flex-wrap gap-2 w-full md:w-auto">
                        <input type="text" x-model="search" @input.debounce.500ms="fetchHistory(1)"
                            placeholder="Search files..."
                            class="bg-black/50 border border-box-border rounded px-3 py-1.5 text-sm text-body focus:border-green-500 outline-none w-full md:w-48">

                        <select x-model="riskFilter" @change="fetchHistory(1)"
                            class="bg-black/50 border border-box-border rounded px-3 py-1.5 text-sm text-body focus:border-green-500 outline-none">
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="text-sm">
                            <template x-if="isLoading">
                                <tr>
                                    <td colspan="6" class="p-8 text-center text-body">
                                        <div
                                            class="w-6 h-6 border-2 border-green-500 border-t-transparent rounded-full animate-spin mx-auto mb-2">
                                        </div> Loading...
                                    </td>
                                </tr>
                            </template>
                            ...
                            <template x-if="!isLoading && history.length === 0">
                                <tr>
                                    <td colspan="6" class="p-8 text-center text-body opacity-60">No analysis jobs found.
                                    </td>
                                </tr>
                            </template>
                                ...
                                <tr class="border-b border-box-border/30 hover:bg-white/5 transition-colors group">
                                    ...
                                    <td class="p-3">
                                        <div class="flex items-center gap-2">
                                            <svg class="w-4 h-4 text-body flex-shrink-0" fill="none" viewBox="0 0 24 24"
                                                stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                            </svg>
                                            <div>
                                                <span class="text-heading-3 text-sm block truncate max-w-[180px]"
                                                    x-text="job.files?.[0]?.original_name || (job.file_count + ' file(s)')"></span>
                                                <span x-show="job.file_count > 1" class="text-body text-xs"
                                                    x-text="'+' + (job.file_count - 1) + ' more'"></span>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="p-3">
                                        <span x-show="job.report && job.report.risk_level"
                                            class="px-2 py-0.5 rounded text-xs font-bold" :class="{
                                    ...
                                    <td class="p-3 text-right">
                                        <div class="flex items-center justify-end gap-3">
                                            <a x-show="job.status === 'completed'" :href="'/analysis/' + job.id"
                                                class="text-green-400 hover:text-green-300 text-xs font-semibold">View</a>
                                            <a x-show="job.status === 'completed' && job.report?.pdf_path"
                                                :href="'/api/analysis/' + job.id + '/report'" target="_blank"
                                                class="text-body hover:text-heading-2 text-xs font-semibold flex items-center gap-1">
                                            ...
                                            <a x-show="job.status === 'completed' && job.report?.pdf_path"
                                                :href="'/analysis/' + job.id + '/export-zip'"
                                                class="text-body hover:text-heading-2 text-xs font-semibold flex items-center gap-1">
                                                <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24"
                                                    stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="mt-6 flex items-center justify-between" x-show="lastPage > 1">
                    <button @click="fetchHistory(currentPage - 1)" :disabled="currentPage === 1"
                        class="btn-secondary text-xs px-3 py-1 disabled:opacity-50">Prev</button>
                    <span class="text-xs text-body">Page <span x-text="currentPage"></span> of <span
                            x-text="lastPage"></span></span>
                    <button @click="fetchHistory(currentPage + 1)" :disabled="currentPage === lastPage"
                        class="btn-secondary text-xs px-3 py-1 disabled:opacity-50">Next</button>
                </div>
